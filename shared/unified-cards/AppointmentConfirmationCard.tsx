...
import { CheckCircle2, Copy, ExternalLink, MapPin, Phone, User, Wrench, Calendar, Clock } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import type { AppointmentConfirmationCardPayload } from './types';

interface AppointmentConfirmationCardProps {
  payload: AppointmentConfirmationCardPayload;
  onAction?: (action: string, payload?: Record<string, unknown>) => void;
  onDismiss?: () => void;
}

export function AppointmentConfirmationCard({ payload, onAction, onDismiss }: AppointmentConfirmationCardProps) {
    ...
    <Card className="w-full border-green-200 bg-gradient-to-br from-white to-green-50/30 shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">{payload.title}</h3>
            {booking.confirmationNumber && (
              <p className="text-green-100 text-sm">Confirmation #{booking.confirmationNumber}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <CardContent className="p-4 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {booking.estimatedArrival && (
                <div className="text-xs text-green-600 mt-1">Est. arrival: {booking.estimatedArrival}</div>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-2 pt-2">
          <Button variant="outline" size="sm" onClick={copyConfirmation} className="flex-1">
            <Copy className="w-4 h-4 mr-1" />
            Copy Details
          </Button>

          {payload.cta && (
            <Button onClick={handleCta} size="sm" className="flex-1 bg-green-600 hover:bg-green-700">
              {payload.cta.label}
              <ExternalLink className="w-4 h-4 ml-1" />
            </Button>
          )}

          {onDismiss && !payload.cta && (
            <Button variant="ghost" size="sm" onClick={onDismiss} className="flex-1 text-gray-500">
              Close
            </Button>
          )}
        </div>
