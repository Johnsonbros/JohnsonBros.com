...
import { Check, Clock, Loader2, Moon, Sun, Sunrise, Sunset, Users } from 'lucide-react';
import { format, parseISO } from 'date-fns';
import type { TimePickerCardPayload, TimeSlot } from './types';

interface TimePickerCardProps {
  payload: TimePickerCardPayload;
...
export function TimePickerCard({ payload, onSelectSlot, onDismiss, isLoading }: TimePickerCardProps) {
  const [selectedSlotId, setSelectedSlotId] = useState<string | null>(payload.selectedSlot || null);

  useEffect(() => {
    setSelectedSlotId(payload.selectedSlot || null);
  }, [payload.selectedSlot]);

  const formattedDate = payload.selectedDate
    ? format(parseISO(payload.selectedDate), 'EEEE, MMMM d')
    : 'Selected Date';

  const slots = payload.slots && payload.slots.length > 0 ? payload.slots : DEFAULT_SLOTS;
    ...
    <Card className="w-full border-blue-200 bg-gradient-to-br from-white to-blue-50/30 shadow-lg">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-sm text-gray-600">
          {payload.message || `Select a time window for ${formattedDate}`}
        </CardDescription>
      </CardHeader>
