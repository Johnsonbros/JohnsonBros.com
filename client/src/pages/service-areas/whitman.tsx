...
export default function WhitmanPlumbing() {
  ...
  const breadcrumbs = [
    ...
    { name: "Whitman", url: "https://www.thejohnsonbros.com/service-areas/whitman" }
  ];
          ...
          <section className="py-16 bg-white">
            <div className="container mx-auto px-4 max-w-6xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 text-center">
                {["Abington", "Rockland", "Hanson", "Halifax", "East Bridgewater", "Brockton"].map((city) => (
                  <Link key={city} href={`/service-areas/${city.toLowerCase().replace(' ', '-')}`} className="text-johnson-blue hover:text-johnson-teal font-medium">{city}</Link>
                ))}
              </div>
            </div>
          </section>
