...
export default function HansonPlumbing() {
  ...
  const localFAQs = [
    { question: "Do you provide emergency plumbing services in Hanson, MA?", answer: "Yes, Johnson Bros. provides 24/7 emergency plumbing services throughout Hanson, MA." },
    { question: "What areas of Hanson do you service?", answer: "We service all of Hanson and surrounding areas." },
    ...
    { question: "Are you licensed to work in Hanson?", answer: "Yes, we are fully licensed master plumbers in Massachusetts." }
  ];
  const breadcrumbs = [
  ...
  const localReviews = [
    { author: "Dan S. - Hanson", rating: 5, datePublished: "2024-11-08", reviewBody: "Quick response and professional service!" },
    { author: "Susan L. - Hanson", rating: 5, datePublished: "2024-10-12", reviewBody: "Fixed our drain issue fast. Great work." }
  ];
  const services = [
    ...
    { name: "Water Heater Service Hanson", description: "Water heater repair and installation" },
    { name: "Pipe Repair Hanson", description: "Expert pipe repair and replacement" }
  ];
  ...
  return (
    <>
      ...
      <ServiceAreaSchema areaName="Hanson" services={services} />
      <FAQSchema questions={localFAQs} />
      <BreadcrumbSchema items={breadcrumbs} />
      <ReviewSchema reviews={localReviews} aggregateRating={{ ratingValue: 4.8, reviewCount: 35 }} />
        ...
        <main className="flex-grow">
            ...
            <div className="container mx-auto px-4">
              <div className="max-w-4xl mx-auto text-center">
                <div className="flex items-center justify-center gap-3 mb-6">
                  ...
                  <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold">Plumber in Hanson, MA</h1>
                </div>
                <p className="text-xl sm:text-2xl mb-8 text-blue-100">Your trusted local plumbing experts serving Hanson and the South Shore</p>
          ...
          <section className="py-16 bg-white">
            <div className="container mx-auto px-4">
              <div className="max-w-6xl mx-auto">
                <h2 className="text-3xl sm:text-4xl font-bold text-center mb-12">Complete Plumbing Services in Hanson, MA</h2>
          ...
          <section className="py-16 bg-gray-50">
            <div className="container mx-auto px-4 max-w-6xl">
              <h2 className="text-3xl font-bold text-center mb-12">Why Hanson Chooses Johnson Bros.</h2>
              <div className="grid md:grid-cols-3 gap-8">
                {[{ icon: Clock, title: "Fast Response", desc: "Same-day service" }, { icon: Shield, title: "Licensed & Insured", desc: "MA master plumbers" }, { icon: CheckCircle, title: "Local Experts", desc: "We know Hanson" }].map((f, i) => (<div key={i} className="text-center p-6 bg-white rounded-lg"><f.icon className="h-12 w-12 text-johnson-blue mx-auto mb-4" /><h3 className="text-xl font-semibold mb-3">{f.title}</h3><p className="text-gray-600">{f.desc}</p></div>))}
              </div>
            </div>
          </section>
          <section className="py-16 bg-white">
            <div className="container mx-auto px-4 max-w-6xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 text-center">
                {["Whitman", "Rockland", "Pembroke", "Halifax", "East Bridgewater", "Abington"].map((c) => (<Link key={c} href={`/service-areas/${c.toLowerCase().replace(' ', '-')}`} className="text-johnson-blue hover:text-johnson-teal font-medium">{c}</Link>))}
              </div>
            </div>
          </section>
          <section className="py-16 bg-gradient-to-br from-johnson-blue to-johnson-teal text-white">
            <div className="container mx-auto px-4 text-center">
              <h2 className="text-3xl sm:text-4xl font-bold mb-6">Need a Plumber in Hanson Today?</h2>
              <p className="text-xl mb-8 text-blue-100">Call Hanson's trusted plumbing experts</p>
