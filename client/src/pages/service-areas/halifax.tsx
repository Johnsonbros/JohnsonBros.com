          ...
          <section className="py-16 bg-white">
            <div className="container mx-auto px-4 max-w-6xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 text-center">
                {["Hanson", "Whitman", "Pembroke", "Kingston", "Plympton", "East Bridgewater"].map((c) => (<Link key={c} href={`/service-areas/${c.toLowerCase().replace(' ', '-')}`} className="text-johnson-blue hover:text-johnson-teal font-medium">{c}</Link>))}
              </div>
            </div>
          </section>
